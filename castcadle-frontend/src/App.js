...
function App() {
  ...
  useEffect(() => {
    ...
    if (!storedUserId) {
      storedUserId = 'xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx'.replace(/[xy]/g, (c) => {
  ...
  useEffect(() => {
    axios.get('https://api.castcadle.com/api/people')
      .then((response) => {
  ...
  useEffect(() => {
    if (userId) {
      axios.get(`https://api.castcadle.com/api/user-data?userId=${userId}`)
        .then(response => {
  ...
  useEffect(() => {
    if (gameStatus !== "INCOMPLETE") {
      axios.get('https://api.castcadle.com/api/correct-person')
        .then(response => {
  ...
  const handleSubmit = () => {
    ...
    axios.post('https://api.castcadle.com/api/guess', { userId: currentUserId, name: selectedPerson })
      .then((response) => {
        ...
        setSelectedPerson('');
        setSearchTerm('');
        axios.get(`https://api.castcadle.com/api/user-data?userId=${currentUserId}`)
          .then(response => {
            setUserGameData(response.data);
          })
          .catch(error => console.error("Error fetching user game data:", error));
